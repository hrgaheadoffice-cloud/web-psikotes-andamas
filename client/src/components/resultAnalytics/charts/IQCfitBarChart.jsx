...
const COLORS = ['#fb923c', '#f97316', '#ea580c', '#c2410c', '#9a3412', '#7c2d12', '#431407'];
...
const gradientId = (index) => `cfit-bar-grad-${index}`;
...
const ActiveBar = (props) => {
  const { fill } = props;
  return (
    <g style={{ filter: 'drop-shadow(0px 10px 14px rgba(234, 88, 12, 0.35))' }}>
      <Rectangle {...props} fill={fill} />
    </g>
  );
};

const IQCfitBarChart = ({ filters }) => {
  ...
  const handleDownloadJpg = useCallback(async (event) => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.download = `IQ-CFIT-${today}.jpg`;
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
      console.error('Failed to download IQ CFIT chart image', err);
    } finally {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(false);
      try {
        const response = await api.get('/analytics/personality/iq-cfit', { params: filters });
        setData(response.data.distribution);
      } catch (error) {
        setError(true);
        console.error('Error fetching IQ CFIT data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div ref={cardRef} className="relative flex h-full w-full flex-col">
      ...
      <ChartWrapper title="IQ CFIT (Logic)" isLoading={loading} isError={error}>
          ...
          <div className="min-h-0 w-full flex-1">
            <ResponsiveContainer width="100%" height={255}>
              <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                ...
                <Tooltip content={<CustomTooltip total={total} />} cursor={{ fill: '#fff7ed' }} />
                <Bar
                  dataKey="value"
                  radius={[10, 10, 4, 4]}
                  isAnimationActive
                ...
                >
                  ...
                  <LabelList dataKey="value" position="top" fill="#78350f" fontSize={11} fontWeight={700} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
