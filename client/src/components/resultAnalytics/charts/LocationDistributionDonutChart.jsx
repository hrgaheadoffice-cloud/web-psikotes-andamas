...
const COLORS = ['#0ea5e9', '#10b981', '#f59e0b', '#ec4899', '#8b5cf6', '#ef4444', '#14b8a6', '#f97316', '#6366f1', '#84cc16', '#d946ef', '#06b6d4', '#a16207', '#64748b'];
...
export default function LocationDistributionDonutChart({ filters }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await api.get('/analytics/overview/location_distribution', { params: filters });
        if (active) setData(Array.isArray(response.data) ? response.data : []);
      } catch {
        if (active) setError('Failed to fetch location data');
      } finally {
  ...
  const handleDownloadJpg = useCallback(async (event) => {
    ...
    try {
      ...
      link.download = `Location-Distribution-${new Date().toISOString().slice(0, 10)}.jpg`;
  ...
  return (
    <div ref={cardRef} className="relative flex h-full w-full flex-col">
      ...
      <ChartWrapper title="Location Distribution" isLoading={loading} isError={error} hasData={data.length > 0}>
