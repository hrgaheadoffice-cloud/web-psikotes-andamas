...
// Palet sequential biru (terang -> gelap), rank 1 paling terang sampai rank 10 paling gelap.
const BASE_COLORS = ['#60a5fa', '#3b82f6', '#2563eb', '#1d4ed8', '#1e40af', '#1e3a8a'];
const DARK_COLORS = ['#2563eb', '#1d4ed8', '#1e40af', '#1e3a8a', '#172554', '#0f172a'];

const gradientId = (index) => `position-bar-grad-${index}`;

const groupTopPositions = (items) => {
    ...
    (Array.isArray(items) ? items : []).forEach((item) => {
        ...
        if (existing) {
            ...
            grouped.set(key, { name: rawName.replace(/\s+/g, ' '), value });
        }
    });
...
const PositionTooltip = ({ active, payload, label, total }) => {
    ...
    return (
        <div className="rounded-xl border border-gray-100 bg-white/95 px-4 py-3 shadow-lg shadow-gray-200/60 backdrop-blur-sm">
            <div className="flex items-center gap-2">
                ...
                <p className="max-w-[220px] text-wrap text-sm font-semibold text-gray-800">{label}</p>
            </div>
...
const TruncatedYAxisTick = ({ x, y, payload }) => {
    ...
    const limit = 18;
    const truncated = label.length > limit ? `${label.substring(0, limit)}...` : label;

    return (
        <g transform={`translate(${x},${y})`}>
            <title>{label}</title>
            <text x={0} y={0} dy={3} textAnchor="end" fill="#64748b" fontSize={10}>
                {truncated}
            </text>
        </g>
    );
...
const ActiveBar = (props) => {
    const { fill } = props;
    return (
        <g style={{ filter: 'drop-shadow(4px 0px 12px rgba(37, 99, 235, 0.35))' }}>
            <Rectangle {...props} fill={fill} />
        </g>
    );
};

const PositionBarChart = ({ filters }) => {
    ...
    const handleDownloadJpg = useCallback(async (event) => {
        ...
        try {
            ...
            link.href = dataUrl;
            link.download = `Top-10-Positions-${today}.jpg`;
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Failed to download Top 10 Positions chart image', error);
        } finally {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setIsError(null);
            try {
                const response = await api.get('/analytics/overview/position', { params: filters });
                setData(response.data);
            } catch {
                setIsError('Failed to fetch position data');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const grouped = useMemo(() => groupTopPositions(data), [data]);

    // Dihitung dari data yang sudah ada (bukan data baru/dummy), dipakai untuk
    // badge persentase di tooltip & legend.
    const total = useMemo(() => grouped.reduce((sum, entry) => sum + Number(entry.value || 0), 0), [grouped]);
    ...
    return (
        <div ref={cardRef} className="relative flex h-full w-full flex-col">
            ...
            <ChartWrapper title="Top 10 Positions" isLoading={isLoading} isError={isError} hasData={chartData.length > 0}>
                    ...
                    <div className="h-[230px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart layout="vertical" data={chartData} margin={{ top: 4, right: 28, left: 4, bottom: 4 }}>
                                ...
                                <YAxis type="category" dataKey="name" width={118} tick={<TruncatedYAxisTick />} axisLine={false} tickLine={false} />
                                <Tooltip content={<PositionTooltip total={total} />} cursor={{ fill: '#eff6ff' }} />
                                <Bar
                                    dataKey="value"
                                    radius={[0, 10, 10, 0]}
                                    name="Peserta"
