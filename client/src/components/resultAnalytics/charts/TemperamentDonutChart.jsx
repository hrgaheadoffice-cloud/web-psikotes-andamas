...
const COLOR_MAP = {
  Choleric: '#f97316', // orange-red
  Melancholic: '#14b8a6', // green-teal
  Phlegmatic: '#f59e0b', // orange
  Sanguine: '#06b6d4', // turquoise
};
...
const gradientId = (name) => `temp-grad-${name.replace(/\s+/g, '-')}`;
...
const TemperamentDonutChart = ({ filters }) => {
  ...
  // GenderPieChart.jsx / MemoryDonutChart.jsx.
  ...
  const handleDownloadJpg = useCallback(async (event) => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.download = `Temperament-${today}.jpg`;
      document.body.appendChild(link);
      link.click();
      link.remove();
    } catch (err) {
      console.error('Failed to download Temperament chart image', err);
    } finally {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(false);
      try {
        const response = await api.get('/analytics/personality/temperament', { params: filters });
        const normalizedData = (response.data.distribution || []).map((entry) => ({
          ...entry,
          name: TEMPERAMENT_MAP[entry.name] || entry.name,
        }));
  ...
  return (
    <div ref={cardRef} className="relative flex h-full w-full flex-col">
      ...
      <ChartWrapper title="Temperament" isLoading={loading} isError={error}>
...
export default TemperamentDonutChart;