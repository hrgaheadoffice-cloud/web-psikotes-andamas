...
// Palet sequential nuansa emerald/teal (terang -> gelap), cocok buat jenjang
// pendidikan. Cycle otomatis kalau kategorinya lebih banyak dari palet.
const BASE_COLORS = ['#34d399', '#10b981', '#2dd4bf', '#14b8a6', '#22c55e', '#059669'];
const DARK_COLORS = ['#059669', '#047857', '#0d9488', '#0f766e', '#15803d', '#065f46'];

const gradientId = (index) => `edu-bar-grad-${index}`;

// Kategori yang dianggap "data tidak diisi" khusus untuk chart Pendidikan ini.
// Endpoint /analytics/overview/education TIDAK mengirim kategori "Tidak Mengisi"
// terpisah (sudah dicek langsung dari response API) — peserta yang tidak mengisi
// pendidikan digabung backend ke dalam kategori "Lainnya". Jadi di sini "Lainnya"
// yang ditandai `*`, bukan "Tidak Mengisi" seperti di Gender/Usia. Ini murni
// penandaan tampilan (label saja) — nilai/`name` asli dari data tidak diubah.
const MISSING_LABEL = 'Lainnya';
const displayCategoryLabel = (name) => (name === MISSING_LABEL ? `${name}*` : name);

const EducationTooltip = ({ active, payload, label, total }) => {
...
const ActiveBar = (props) => {
    const { fill } = props;
    return (
        <g style={{ filter: 'drop-shadow(0px 10px 14px rgba(5, 150, 105, 0.35))' }}>
            <Rectangle {...props} fill={fill} />
        </g>
    );
};

const EducationBarChart = ({ filters }) => {
    ...
    // download. Same self-contained pattern as GenderPieChart.jsx / AgeBarChart.jsx.
    ...
    const handleDownloadJpg = useCallback(async (event) => {
        ...
        try {
            ...
            link.href = dataUrl;
            link.download = `Education-Distribution-${today}.jpg`;
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Failed to download Education Distribution chart image', error);
        } finally {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setIsError(null);
            try {
                const response = await api.get('/analytics/overview/education', { params: filters });
                setData(response.data);
            } catch {
                setIsError('Failed to fetch education data');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    return (
        <div ref={cardRef} className="relative flex h-full w-full flex-col">
            ...
            <ChartWrapper title="Distribusi Pendidikan" isLoading={isLoading} isError={isError} hasData={data.length > 0}>
                    ...
                    <div className="h-[286px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={chartData} margin={{ top: 24, right: 12, left: -18, bottom: 4 }}>
                                ...
                                <Tooltip content={<EducationTooltip total={total} />} cursor={{ fill: '#f8fafc' }} />
                                <Bar
                                    dataKey="value"
                                    radius={[10, 10, 4, 4]}
                                    name="Peserta"
                                ...
                                >
                                    ...
                                    <LabelList dataKey="value" position="top" fill="#475569" fontSize={10} fontWeight={700} />
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
